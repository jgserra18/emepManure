...
Animal = R6Class("Animal",
  public = list(
    ...
    initialize = function(animal_type, config_dir = file.path(getwd(), "inst/extdata")) {
    ...
    #' @description Get emission factors specifically for housing components
    #' @return A list of emission factors for housing
    get_housing_emission_factors = function() {
      housing_factors = list()
      
      # Extract housing-specific NH3 emission factors for both solid and slurry
      if (!is.null(self$emission_data$nh3)) {
        if (!is.null(self$emission_data$nh3$solid) && !is.null(self$emission_data$nh3$solid$housing)) {
          housing_factors$solid = list()
          housing_factors$solid$nh3 = self$emission_data$nh3$solid$housing
        }
        
        if (!is.null(self$emission_data$nh3$slurry) && !is.null(self$emission_data$nh3$slurry$housing)) {
          housing_factors$slurry = list()
          housing_factors$slurry$nh3 = self$emission_data$nh3$slurry$housing
        }
      }
      
      # Add excretion data which is relevant for housing calculations
      if (!is.null(self$emission_data$excretion)) {
        housing_factors$excretion = self$emission_data$excretion
      }
      
      # Add straw/litter data which is relevant for housing
      if (!is.null(self$emission_data$straw)) {
        housing_factors$straw = self$emission_data$straw
      }
      
      return(housing_factors)
    },
    ...
    #' @description Get excretion data for the animal (housing component)
    #' @return A list containing excretion data
    get_excretion_data = function() {
      if (!is.null(self$emission_data$excretion)) {
        return(self$emission_data$excretion)
      } else {
        warning("No excretion data available for ", self$animal_type)
        return(list())
      }
    },
    
    #' @description Get straw/litter data for the animal (feeding component)
    #' @return A list containing straw/litter data
    get_straw_data = function() {
      if (!is.null(self$emission_data$straw)) {
        return(self$emission_data$straw)
      } else {
        warning("No straw/litter data available for ", self$animal_type)
        return(list())
      }
    },
